...
T2 = [-2, 0, 2]';
T3 = [1, 1, 1]';
disp('The input signal vectors:');
disp(s1_new);
disp(s2_new);
disp(s3_new);
disp('Normalized input signal vectors:');
...
disp(cosine_matrix);
disp('The associative weight matrix:');
w1 = T1 * s1_new_norm';
w2 = T2 * s2_new_norm';
w3 = T3 * s3_new_norm';
...
disp('Similarity between responses and corresponding targets:')
disp(cosine_sim1);
disp(cosine_sim2);
disp(cosine_sim3);